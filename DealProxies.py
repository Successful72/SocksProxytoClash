#!/usr/bin/env python3
"""
Socks5代理处理脚本：DealProxies.py
功能：从 txt 直链拉取 socks5 代理列表，查询 IP 地理位置，生成 Clash Meta 可识别的 proxies YAML 配置文件。
...
def get_geo(ip: str) -> str:
    """查询 IP 归属地，返回 'Country-City' 格式字符串。"""
    if ip in GEO_CACHE:
        return GEO_CACHE[ip]
    try:
        ...
        if data.get("status") == "success":
            country = data.get("country", "Unknown").replace(" ", "")
        ...
        result = "Unknown"
    GEO_CACHE[ip] = result
    # ip-api.com 免费限额 45 req/min，适当限速
    time.sleep(1.4)
    return result


def build_clash_proxy(proxy: dict, index: int) -> dict:
    """
    构造单条 Clash Meta socks5 代理条目。
    命名格式：geo-Country-City-HH:MM（例：geo-UnitedStates-NewYork-14:10）
    """
...
def generate_yaml(proxies_raw: list[dict]) -> str:
    ...
    for p in proxies_raw:
        ...
        hhmm = p["timestamp"].split(" ")[-1]
        base_name = f"geo-{geo}-{hhmm}"
